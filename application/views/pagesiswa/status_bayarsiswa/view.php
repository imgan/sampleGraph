    ...
    <form class="form-horizontal" role="form" id="formSearch">
        <div class="form-group">
            <div class="col-xs-12 col-md-5 col-lg-5">
                <input type="hidden" name="siswa" id="siswa" value="<?php echo $this->session->userdata('nis'); ?>">
                <!-- <select class="form-control" name="siswa1" id="siswa1">
    ...
    if ($("#formSearch").length > 0) {
        $("#formSearch").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo site_url('modulsiswa/status_bayarsiswa/search_siswa') ?>',
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo site_url('modulsiswa/status_bayarsiswa/view_list_status') ?>',
                    data: $('#formSearch').serialize(),
                    async: true,
                    dataType: 'json',
                    success: function(data) {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            var nominal_bayar = (data[i].nominal_bayar == null ? '0' : data[i].nominal_bayar);
                            var status_pembayaran = (data[i].status_pembayaran == null ? '0' : data[i].status_pembayaran);
                            var decode_status_pembayaran = (status_pembayaran == 'L' ? 'Lunas' : status_pembayaran);
                            var nominal_lunas = (data[i].nominal_lunas == null ? '0' : data[i].nominal_lunas);
                            action_edit = '';
                            action_hapus = '';
                            if(data[i].kelas != "-"){
    ...
    if ($("#formEdit").length > 0) {
        $("#formEdit").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('modulsiswa/status_bayarsiswa/update_lunas') ?>",
                    type: "POST",
    ...
    $('#show_data').on('click', '.item_hapus', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('modulsiswa/status_bayarsiswa/delete') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
