                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Nama Kelas </label>
                                <div class="col-sm-6">
                                    <input type="text" id="e_nama_kelas" name="e_nama_kelas" id="form-field-1" placeholder="IX" class="form-control" />
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    if ($("#formTambah").length > 0) {
        $("#formTambah").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('jeniskelas/simpan') ?>",
                    type: "POST",
    ...
    if ($("#formEdit").length > 0) {
        $("#formEdit").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('jeniskelas/update') ?>",
                    type: "POST",
...
<script type="text/javascript">
    ...
    function show_data() {
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url('jeniskelas/tampil') ?>',
            async: true,
            dataType: 'json',
            success: function(data) {
                ...
                for (i = 0; i < data.length; i++) {
                    html += '<tr>' +
                        ...
                        '<button  href="#my-modal-edit" class="btn btn-xs btn-info item_edit" title="Edit" data-id="' + data[i].id_kelas + '">' +
                        '<i class="ace-icon fa fa-pencil bigger-120"></i>' +
                        '</button> &nbsp' +
                        '<button class="btn btn-xs btn-danger item_hapus" title="Delete" data-id="' + data[i].id_kelas + '">' +
    ...
    $('#show_data').on('click', '.item_edit', function() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('jeniskelas/tampil_byid') ?>",
            async: true,
            dataType: "JSON",
            data: {
                id: id,
            ...
            success: function(data) {
                $('#e_id').val(data[0].id_kelas);
                $('#e_nama_kelas').val(data[0].nama);
            }
        });
    });
    ...
    $('#show_data').on('click', '.item_hapus', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('jeniskelas/delete') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
