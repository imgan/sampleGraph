...
class Lap_rekapbayarsiswa extends CI_Controller
{
    ...
    public function showsiswa()
    {
        ...
        $my_data = $this->model_rekapbayarsiswa->getsiswa($ps)->result_array();
        echo "<option value='0'>--Pilih Siswa --</option>";
        foreach ($my_data as $value) {
            echo "<option value='" . $value['NOREG'] . "'>[" . $value['NOREG'] .'-'. $value['NMSISWA']."] </option>";
        }
    }
    ...
    public function search()
    {
        ...
        $data = $this->model_rekapbayarsiswa->getsiswabyid($siswa)->result_array();
        echo json_encode($data);
    }

    public function index()
    {   
        $myps = $this->model_rekapbayarsiswa->viewOrdering('tbps','id','desc')->result_array();
        $siswa = $this->model_rekapbayarsiswa->viewOrdering('mssiswa','id' ,'desc');
        $data = array(
            'page_content'     => '../pagekasir/lap_rekapbayarsiswa/view',
            'ribbon'         => '<li class="active">Laporan Rekap Pembayaran Siswa</li>',
